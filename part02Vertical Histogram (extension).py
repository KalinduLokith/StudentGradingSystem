...
while True:  #this is infinite loop or endless loop
 ...
 if(choise.lower()=='y'):
    ...
    max_no = max(progress,trailer,retriever,Exclude)

    print('_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _\n Vertical Histogram ')
           

    print(f"{'Progress' : <10}{'Trailer' : ^10}{'Retriever' : ^10}{'Exclude' : >5}")

    for x in range(max_no):

        print(f"{printStars(progress,x): ^10}{printStars(trailer,x) : ^10}{printStars(retriever,x) : ^10}{printStars(Exclude,x) : ^10}")
